...
<select class="form-select menu-post-type">
  ...
  <option value="custom">Custom</option>
        ...
        <div class="col-6">
          <input class="form-control menu-search" placeholder="Search">
          <input class="form-control menu-custom-link d-none" placeholder="Link">
          <div class="position-relative">
            <ul class="list-group mt-2 menu-selected d-none position-absolute z-10">
          </ul>
          </div>
        </div>
        <div class="col-2"><input class="form-control menu-label" placeholder="Label"></div>
...
let maxIds = menuItems.map(mi=>JSON.parse(mi)).map(mi=>mi.menuId);
$this.maxId =  Math.max(...maxIds);
  console.log("Updateview(): selectedMenus");
  console.log(menuItems);
  for(let sm of menuItems){
    let dm = JSON.parse(sm);
    console.log(dm);
    addMenus(dm);
  }
  }
...
if(!$this.main.vu.required(menuControl,$this.menuLabelControl)){
  return;
}
if(isCustom){
  $this.selectedMenu = {
...
console.log("smd");
console.log(smd);
let row = $this.main.pbu.createElement('div',['d-flex','my-2','border','border-primary']);
$this.main.pbu.appendChild($this.displaySection,row);
row.innerHTML = 
`
<div  class="me-4"><p class="menu-label">${selectedMenu.title}</p></div>
<div  class="mx-2"><button class="menu-edit">Edit</button></div>
<div  class="mx-2"><button class="menu-delete">Delete</button></div>
`;
row.setAttribute("draggable","true");
let menuLabel = row.querySelector('p.menu-label');
$this.main.pbu.setAttribute(menuLabel,'data-selected-menu',smd);
let menuEdit = row.querySelector('.menu-edit');
let menuDelete = row.querySelector('.menu-delete');
//Edit
$this.main.pbu.listen(menuEdit,'click',()=>{
let modalBody = $this.main.pbu.createElement('input',['form-control']);
let modal = $this.main.utils.setModal('Change Label',modalBody);
//
$this.main.pbu.listen(modal.confirm,'click', ()=>{
let newLabel = modalBody.value;
  selectedMenu.title = newLabel;
  menuLabel.textContent = newLabel;
  let smd = JSON.stringify(selectedMenu);
  $this.main.pbu.setAttribute('data-selected-menu',smd,menuLabel);
  modal.dismiss.click();
});
});
...
let modal = $this.main.utils.setModal('Deletion Menu',null);
$this.main.pbu.listen(modal.confirm,'click', ()=>{
row.remove();
modal.dismiss.click();
});
